...
public class Weather : MonoBehaviour
{
    ...
    //GameObject input = GameObject.Find("ChangeSeasonObject");
    //ChangeSeason cs = input.GetComponent<ChangeSeason>();
    ...
    public IEnumerator GetWeatherIntput(string a)
    {
        ...
        GameObject go = GameObject.Find("ChangeSeasonObject");
        ChangeSeason cs = go.GetComponent<ChangeSeason>();
        a = wp.yo;
        ...
        else
        {
            ...
            Debug.Log("this is t: " + t);
             if(t < 50)
            {
                Debug.Log("Warp to Winter");
                    cs.WarpToWinterWeather();
            }
            else if(t >= 50 && t <= 70) {
                Debug.Log("Wrap to Spring");
                    cs.WarpToSpringWeather();
                    //SceneManager.LoadScene(1);
                }
            else if(t >= 90 && t <= 150) {
                Debug.Log("Wrap to Summer");
                    cs.WarpToSummerWeather();
                    //SceneManager.LoadScene(5);
                }
            else if(t > 70 && t < 90) {
                Debug.Log("Wrap to Fall");
                    cs.WarpToFallWeather();
